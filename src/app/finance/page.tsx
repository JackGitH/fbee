...
const translations = {
  zh: {
    ...
    systemStatus: '系统状态',
    efficiency: '效率',
    minerEfficiencyDesc: '您的矿机目前正以最高效率运行。下次奖励在',
    withdrawEarnings: '提取收益',
    referralHashrateBoost: '推荐算力加成',
    ...
    recentActivity: '最近活动',
    powerAnalysis: '功率分析',
    powerEfficiency: '功率效率',
    inviteMoreFriends: '再邀请 2 位好友即可达到',
    masterLevel: '大师等级',
    unlock4xRewards: '并解锁 4 倍奖励',
    rewardDistribution: '奖励分配',
    stakeConfirmed: '质押确认',
    autoReInvest: '自动复投',
  },
  en: {
    totalStakedCapital: 'Total Staked Capital',
    reInvest: 'Re-invest',
    ...
    minerEfficiencyDesc: 'Your miners are currently operating at peak efficiency. Next reward in',
    withdrawEarnings: 'Withdraw Earnings',
    referralHashrateBoost: 'Referral Hashrate Boost',
    ...
    inviteMoreFriends: 'Invite 2 more friends to reach',
    masterLevel: 'Master Level',
    unlock4xRewards: 'and unlock 4x rewards.',
    rewardDistribution: 'Reward Distribution',
    stakeConfirmed: 'Stake Confirmed',
    autoReInvest: 'Auto Re-invest',
  },
} as const;
    ...
    <main className="pt-24 pb-32 px-4 lg:px-8 max-w-7xl mx-auto space-y-8">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <GlassCard className="relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              ...
              <div className="flex gap-4">
                <Button variant="secondary" size="lg">{t.reInvest}</Button>
                <Button variant="primary" size="lg">{t.stakeUsdt}</Button>
              </div>
            </div>
          </GlassCard>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-surface-container-low rounded-xl p-5 border border-outline-variant/10">
              <span className="text-xs text-on-surface-variant uppercase tracking-widest font-bold block mb-2">{t.hashPower}</span>
              <CountUp 
              ...
              />
              <div className="text-[10px] text-tertiary mt-1">+12% {t.bonus}</div>
            </div>
            <div className="bg-surface-container-low rounded-xl p-5 border border-outline-variant/10">
              <span className="text-xs text-on-surface-variant uppercase tracking-widest font-bold block mb-2">{t.multiplier}</span>
              <CountUp 
              ...
              />
              <div className="text-[10px] text-on-surface-variant mt-1">{t.levelExpert}</div>
            </div>
            <div className="bg-surface-container-low rounded-xl p-5 border border-outline-variant/10">
              <span className="text-xs text-on-surface-variant uppercase tracking-widest font-bold block mb-2">{t.todayYield}</span>
              <CountUp 
            ...
            <div className="bg-surface-container-low rounded-xl p-5 border border-outline-variant/10">
              <span className="text-xs text-on-surface-variant uppercase tracking-widest font-bold block mb-2">{t.totalEarned}</span>
              <CountUp 
              ...
              />
              <div className="text-[10px] text-on-surface-variant mt-1">{t.fbeeBalance}</div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4">
          <GlassCard className="h-full flex flex-col justify-between border-l-4 border-l-secondary">
            <div className="space-y-4">
              <h3 className="text-xl font-display font-bold">{t.miningStatus}</h3>
              <div className="flex items-center justify-between text-sm">
                <span className="text-on-surface-variant">{t.systemStatus}</span>
              ...
              <div>
                <div className="flex justify-between text-xs mb-1">
                  <span className="text-on-surface-variant">{t.efficiency}</span>
              ...
              <p className="text-xs text-on-surface-variant leading-relaxed">
                {t.minerEfficiencyDesc} <span className="text-on-surface font-mono">{formatTime(nextRewardTime)}</span>。
              </p>
            </div>
            <div className="pt-6">
              <Button variant="primary" fullWidth>{t.withdrawEarnings}</Button>
            </div>
          </GlassCard>
        </div>
      </section>
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-display font-bold tracking-tight">{t.referralHashrateBoost}</h2>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-surface-container-high/50 text-on-surface-variant text-xs uppercase tracking-widest font-bold">
                  <th className="px-6 lg:px-8 py-5 font-bold">{t.level}</th>
                  <th className="px-6 lg:px-8 py-5 font-bold">{t.directRefs}</th>
                  <th className="px-6 lg:px-8 py-5 font-bold">{t.multiplier}</th>
                  <th className="px-6 lg:px-8 py-5 font-bold">{t.bonusHash}</th>
                  <th className="px-6 lg:px-8 py-5 font-bold text-right">{t.status}</th>
                </tr>
              </thead>
                  ...
                  >
                    ...
                    <td className="px-6 lg:px-8 py-5 text-right">
                      {tier.status === 'completed' && (
                        <span className="text-tertiary text-xs bg-tertiary/10 px-2 py-1 rounded-md">{t.unlocked}</span>
                      )}
                      {tier.status === 'current' && (
                        <span className="text-primary text-xs bg-primary/20 px-2 py-1 rounded-md ring-1 ring-primary/30">{t.current}</span>
                      )}
                      {tier.status === 'locked' && (
                        <span className="text-outline text-xs bg-outline-variant/10 px-2 py-1 rounded-md">{t.locked}</span>
                      )}
                    </td>
                  </tr>
                ))}
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <h2 className="text-2xl font-display font-bold tracking-tight">{t.recentActivity}</h2>
          <div className="space-y-3">
            {mockFinanceData.recentActivity.map((activity, index) => {
              const activityType = activity.type === 'Reward Distribution' ? t.rewardDistribution :
                activity.type === 'Stake Confirmed' ? t.stakeConfirmed : t.autoReInvest;
              return (
                <div 
                  key={index}
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="font-bold">{activityType}</div>
        ...
        <div className="space-y-6">
          <h2 className="text-2xl font-display font-bold tracking-tight">{t.powerAnalysis}</h2>
          <GlassCard className="h-[400px] flex flex-col items-center justify-center text-center">
            <div className="mb-6 relative">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                />
                <span className="text-[10px] text-on-surface-variant uppercase tracking-tighter">{t.powerEfficiency}</span>
              </div>
            </div>
            <p className="text-sm text-on-surface-variant mb-6 px-4">
              {t.inviteMoreFriends} <span className="text-primary font-bold">{t.masterLevel}</span> {t.unlock4xRewards}
            </p>
          </GlassCard>
        </div>
      </section>
